...
_C.DATA.TRAIN = ['train2014']   # i.e. trainval35k, AKA train2017
# For now, only support evaluation on single dataset
_C.DATA.VAL = 'minival2014'  # AKA val2017
_C.DATA.NUM_CATEGORY = 7    # 80 categories.
_C.DATA.CLASS_NAMES = ['BG','unknown','cyclist','tricycle','smallMot','bigMot','pedestrian','crowds']  # NUM_CLASS (NUM_CATEGORY+1) strings, to be populated later by data loader. The first is BG.
...
_C.TRAIN.EVAL_PERIOD = 500  # period (epochs) to run eva

# preprocessing --------------------
# Alternative old (worse & faster) setting: 600, 1024
_C.PREPROC.TRAIN_SHORT_EDGE_SIZE = [500, 500]  # [min, max] to sample from
_C.PREPROC.TEST_SHORT_EDGE_SIZE = 500
_C.PREPROC.MAX_SIZE = 700
